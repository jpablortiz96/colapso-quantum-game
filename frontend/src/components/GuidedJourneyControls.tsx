...
export function GuidedJourneyControls() {
  const { gameMode, guidedStep, guidanceActive, guidedError, rewindsRemaining, transcript, universe, dismissGuidance, returnToGuidance, rewindLastAction, reset } = useDailyGameStore();
  if (gameMode !== "GUIDED") return null;
  const journey = getGuidedJourney(universe.universeNumber);
  ...
  const step = journey.steps[guidedStep];
  ...
  return <section aria-label="Controles de Ruta Guiada" className="guided-controls">
    <header><span>RUTA GUIADA · PASO {Math.min(guidedStep + 1, journey.steps.length)} DE {journey.steps.length}</span><button onClick={dismissGuidance} type="button">Cerrar orientación</button></header>
